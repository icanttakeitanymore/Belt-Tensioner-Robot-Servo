...
var r = Math.sqrt(dec * dec + lat * lat), l = (dec * 2) - r;
...
var acc = (run && speed > 1) ? Math.max(0, gLong) : 0;
var lastAcc = root["lastAcc"] || 0;
var kick = (acc > lastAcc) ? (acc - lastAcc) * 30 : 0; // Tune multiplier (30) for punch intensity
root["lastAcc"] = acc;

// Reset persistent cache environments in UI calibration test modes
if ($prop('Settings.max_test') || $prop('Settings.TestOffsets')) {
    root["sl"] = root["sr"] = root["lastL"] = root["lastR"] = root["lastAcc"] = null; return "";
}
...
r = Math.min(tmax | 1, Math.max(3, finalR)) | 1;
